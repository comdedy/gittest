...
#define UI_IS_IPHONE_X          ((IS_IPHONE && [[UIScreen mainScreen] bounds].size.height >= 812))

#define kIs_iPhoneX ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) : NO)
#define iPhoneXSeries   (IS_IPHONE && [[[UIApplication sharedApplication] delegate] window].safeAreaInsets.bottom > 0.0)

//导航栏高度
#define kStatusBarAndNavigationBarHeight (iPhoneXSeries ? 88.f : 64.f)
//状态栏高度
#define kStatusBarHeight (iPhoneXSeries ? 44.f : 20.f)
// Tabbar height.
#define  kTabbarHeight         (iPhoneXSeries ? (49.f+34.f) : 49.f)// Tabbar safe bottom margin.
#define  kTabbarSafeBottomMargin         (iPhoneXSeries ? 34.f : 0.f)
#define  Padding  10
